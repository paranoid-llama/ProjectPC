    ...
    return (
        <Box sx={{width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center'}}>
            ...
            <Tabs value={offerType} onChange={(e, newVal) => changeOfferType(newVal)} sx={{'&.MuiTabs-root': {height: '20px', minHeight: '12px', my: 1}, '& .MuiTab-root': {py: 0, minHeight: '12px'}}}>
                ...
                <Tab label='Item Offers' value='itemOffers' disabled={rates.itemOffers.length === 0}/>
            </Tabs>
            <Box sx={{width: '90%', px: '2%', height: '70%', display: 'flex', flexDirection: 'column', justifyContent: 'start', alignItems: 'center', maxWidth: '500px', backgroundColor: 'rgba(240, 240, 240, 0.9)', boxShadow: '0px 2px 1px -1px rgba(0,0,0,0.2), 0px 1px 1px 0px rgba(0,0,0,0.14), 0px 1px 3px 0px rgba(0,0,0,0.12)'}}>
                {rates[offerType].map((rate, idx) => {
                    ...
                    return (
                        <Box sx={{height, display: 'flex', width: '100%'}} key={`${offerType}-rate-display-${idx+1}`}>
                            <Typography sx={{height: '100%', width: '80%', fontSize: '12px', textAlign: 'start', ...borderBottom}}>
                                {rate.items[0]} : {rate.items[1]}
                            </Typography>
                            <Typography sx={{height: '100%', width: '20%', fontSize: '12px', textAlign: 'end', fontWeight: 700, ...borderBottom}}>
