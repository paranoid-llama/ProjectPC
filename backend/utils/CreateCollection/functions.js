...
function removeBallsOutsideScope(pokemon, ballScope, excludedCombos, importedBallInfo) {
    ...
    if (Array.isArray(newPokemon)) {
        newPokemon.forEach((poke, idx) => {
            ...
            const particularPokemonHasNoBallInfo = pokemon === undefined || importedBallInfo[idx] === undefined || (importedBallInfo[idx] !== undefined && newPokemon !== undefined && importedBallInfo[idx].name !==  newPokemon[idx].name)
